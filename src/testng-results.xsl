...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="testNgXslt.url"/>
	<xsl:param name="testNgXslt.wspaceDir"/>
        ...
        <xsl:result-document href="{testng:absolutePath('main.js')}" format="text">
            ...
            function renderSvgEmbedTag(chartWidth, chartHeight) {
                ...
                if (success) {
                    ...
                    var text = document.createTextNode('SVG Pie Charts are not available. Please install a SVG viewer for your browser or use Firefox.');
    ...
    <xsl:template name="htmlHead">
        <head>
			...
			<script type="text/javascript" src="excellentexport.js"/>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
				...
				<xsl:call-template name="logo"/> 
                    <h2>suites overview</h2>
                     
          <tr>
   			<xsl:call-template name="timeDayMonthYear"/>
        ...
        <div style="width: 500px;">
			...
			<td nowrap="true" align="right">
                <xsl:call-template name="TestScripts"/>
             </td> &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;
             
              <td nowrap="true" align="right">
			  <xsl:call-template name="BuildLogs"/>	
             </td>  &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;
             
              <td nowrap="true" align="right">
			   <xsl:call-template name="ExelReportUrl"/>		  
             </td>
                    ...
                    <table width="100%" style="font-size: 16px; margin-bottom: 10px;" cellspacing="1">
                        <tr>
                            <td width="100%">
                                ...
                                <b>
                                    <xsl:value-of select="testNgXslt.ScreenShots"/>
                                </b>
                            </td>
                            <xsl:call-template name="percentageOverview">
                                <xsl:with-param name="failedCount"
    ...
    <xsl:template name="testMethods">
        ...
        <div style="width: 500px;">
   		  <tr>	                                
			<td>Date:  
			 <xsl:call-template name="timeDayMonthYear"/> CST   
            </td><br></br>
        ...
        <div id="testMethodsByStatus">
            ...
            <table class="testMethodsTable" cellpadding="0" cellspacing="0">
                <tr class="methodsTableHeader">
                    <td style="width:60px">Step</td>
	                <td nowrap="true">Started</td>
                    <td nowrap="true">Duration</td>
                    <td nowrap="true">Exception</td>
					<td style="width:600px">Test Scenarios</td>
					<td style="width:300px">Imput Values</td> <!---added -->
					<td nowrap="true">Status</td> <!---added -->
					<td nowrap="true">ScreenShots</td> <!---added -->
                </tr>
        ...
        <div id="testMethodsByClass">
            ...
            <xsl:for-each select="$classes">
                ...
                <table id="datatable" class="testMethodsTable" cellpadding="0" cellspacing="0">
                    <tr>
                        <td colspan="4">
                    ...
                    <tr class="methodsTableHeader">
	                    <td style="width:60px">Step</td>
		                <td nowrap="true">Started</td>
	                    <td style="width:20px" nowrap="true">Duration</td>
	                    <td nowrap="true">Exception</td>
						<td style="width:600px" >Test Scenarios</td>
						<td style="width:300px" >Input Values</td> <!---added -->
						<td nowrap="true">Status</td> <!---added -->						 
						<td nowrap="true">ScreenShots</td> <!---added -->   
                    </tr>
    ...
    <xsl:template name="testMethodsList">
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td style="width:60px" class="firstMethodCell">
	              ...
	              <td >
                    <xsl:choose>
				         <xsl:when test="./params">
				           <xsl:value-of select="testng:concatParams(./params/param)"/>
				         </xsl:when>
				         <xsl:otherwise>
				          No inputs 
				         </xsl:otherwise>
    ...
    <xsl:template name="powered-by">
        <div style="margin-top: 15px; color: gray; text-align: center; font-size: 9px;">
            Generated with
            <a href="https://code.google.com/p/mvp-evip-xslt-test-suite/" style="color: #8888aa;" target="_blank">
                TestNG XSLT
            </a>
	...
	<xsl:template name="BuildLogs">
              <a href="file:///{$testNgXslt.wspaceDir}/buildlogs/" title="click to ant consol logs">
			  <img src="file:///{$testNgXslt.wspaceDir}/images/clip.jpeg" style="width: 60px; height: 55px;"/></a>			  
    </xsl:template>
    
    <xsl:template name="TestScripts">
              <a href="file:///{$testNgXslt.wspaceDir}/testscripts/" title="click to see Test Plan">
				<img src="file:///{$testNgXslt.wspaceDir}/images/ns.jpg" style="width: 60px; height: 55px;"/></a>
    </xsl:template>
    
    <xsl:template name="ScreenShots">
    <xsl:variable name="dateToday" select="format-date(current-date(), '[M1]-[D1]-[Y0001]')"/>  
              <a href="file:///{$testNgXslt.wspaceDir}/ScreenShots/{$dateToday}/{../@name}/{@name}/" title="click to see screen-shots">		
			  <img src="file:///{$testNgXslt.wspaceDir}/images/icon.png" style="width: 25px; height: 25px;"/>&#160; Pictures</a>		
    </xsl:template>
    
    <xsl:template name="ExelReportUrl">
     <xsl:variable name="dateToday" select="format-date(current-date(), '[M1]-[D1]-[Y0001]')"/>
              <a href="file:///{$testNgXslt.wspaceDir}/exelReport/{$dateToday}" title="Upload Excel to ALM">
			  <img src="file:///{$testNgXslt.wspaceDir}/images/excel.gif" style="width: 60px; height: 55px;"/></a>
    </xsl:template>
    
    <xsl:template name="TodayDate">
              <xsl:value-of select="format-date(current-date(), 
                          '[M01]-[D01]-[Y0001]')"/>
    </xsl:template>
	
	<xsl:template name="logo">
              <img src="file:///{$testNgXslt.wspaceDir}/images/usps.gif" />
			  <b>&#160;&#160;&#160;QUALITY ASSURANCE</b>  
     </xsl:template>
     
     <!-- <xsl:template name="DateTime">
      <xsl:variable name="dateToday" select="format-dateTime(current-dateTime(), '[h01]:[m01] [P] on [FNn] the [D1o].')"/>
      <xsl:variable name="dayofweek" select="format-dateTime(current-dateTime(),'[h1]:[m01] [P] on [MNn] [D].')"/>
      <xsl:variable name="dateToday" select="format-time(current-time(), '[H01]:[m01] [z]')"/>  
     </xsl:template>
      -->
     <xsl:template name="timeDayMonthYear">
     <xsl:value-of select="format-dateTime(current-dateTime(), '[h01]:[m01] [P] on [FNn] , [MNn] the [D1o], [Y0001].')"/> 
     </xsl:template>
